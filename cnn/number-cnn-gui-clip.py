...
model.load_weights("./model/handwriting_cnn.h5")
...
class Application(tk.Frame):
    ...
    def recog(self):
        ...
        if majorVersion>=3: # for Stretch
            ...
            recog_img = recog_img.resize(size=(32,32), resample=Image.BICUBIC)
        else: # for Jessie
            # 描画された数字を切り出し、認識エリア内に最大化
            if self.drawxmin<self.drawxmax and self.drawymin<self.drawymax:
                recog_img = Image.new('L', (w, h), color=255)
            ...
            recog_img = recog_img.resize(size=(32,32), resample=Image.BILINEAR)
        ...
        Ximg = cv2.cvtColor(Ximg.astype(np.float32), cv2.COLOR_GRAY2BGR)
        #0~1の値に変換
        Ximg = Ximg/255.0
        Ximg=Ximg.reshape(1,32,32,3)
        score = model.predict(Ximg)
        # 予測結果を右の領域の表示
        self.digit_canvas.create_text(w/2, h/2, text = '{0:d}'.format(np.argmax(score)), fill='white', font = ('FixedSys', int(w/2)))
...
app.master.title('数字認識（畳み込みニューラルネットワーク版、補正あり）')
app.mainloop()
